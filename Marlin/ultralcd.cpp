...
#ifdef ULTIPANEL
    if (LCD_CLICKED)   ///detail  LCD_CLICKED
    {
...
void lcd_init()              //detail
{
    ...
    digitalWrite(BUTTON_CLICKED, HIGH);  
     button_up_flag=0;
     button_down_flag=0;
//  #if BTN_ENC > 0
...
void lcd_buttons_update()       /////detail
{
#ifdef NEWPANEL
    uint8_t newbutton=0;
    if(READ(BUTTON_UP)==0)
    ...
    else button_up_flag=0;
    if(READ(BUTTON_DOWN)==0)
    {
    ...
    else button_down_flag=0;
    if((blocking_enc<millis()) && (READ(BUTTON_CLICKED)==0))
    newbutton |= EN_C;
      buttons = newbutton;
#endif//!NEWPANEL)
